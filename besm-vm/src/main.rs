...
impl<'a> VM<'a> {
  ...
  pub fn step(&'a mut self) -> () {
    let opcode = self.is[self.next_instr() as usize];

    match Instruction::from_bytes(opcode) {
      Add  { a: l, b: r, c: res, normalize: needs_norm } => {
        let mut lfloat = Float::from_bytes(l);
        let mut rfloat = Float::from_bytes(r);

        match lfloat.exp.cmp(&rfloat.exp) {
          Ordering::Less    => {
            lfloat.mant >>= lfloat.exp - rfloat.exp;
            lfloat.exp = rfloat.exp;
          }
          Ordering::Greater => {
            rfloat.mant >>= rfloat.exp - lfloat.exp;
        ...
        self.set_address(res, val.to_bytes()).increment_ic();
  ...
  fn get_address(&'a self, ix: u64) -> u64 {
    if ix <= 1023 {
  ...
  fn set_address(&'a mut self, ix: u64, val: u64) -> &mut Self {
    if ix <= 1023 {
...
impl Float {
  pub fn from_bytes(word: u64) -> Float {
    ...
    let mant = word.get_bits(0..31) as u32;
  ...
  pub fn to_bytes(&mut self) -> u64 {
    ...
    bytes.set_bits(0..31, self.mant as u64);
...
fn main() {
    ...
    let words : Vec<u64> = iter::repeat_with(|| f.read_u64::<NativeEndian>().unwrap()).take(1023).collect();
