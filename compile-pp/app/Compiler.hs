...
sourceMapDirArg :: S.Parser String
sourceMapDirArg =
  strOption $
    long "source-map-dir"
      <> metavar "SOURCEMAPDIR"
      <> help "directory to write sourcemap files"
...
sourceMapArg =
  some $ strOption $
    long "source-map"
      <> metavar "SOURCEMAP"
      <> help "location of a sourcemap file"
...
options =
  simpleOptions "v0.1.0" "PP-BESM Compiler Source Code and Debugger" "" (pure ()) $ do
    addCommand
      ...
      compileCommand
      ((,) <$> optional outputFileArg <*> optional sourceMapDirArg)
    addCommand
      "trace"
...
traceCommand (inFile, oFile, smFiles, dedup) = do
  visualizeTrace dedup inFile oFile smFiles
